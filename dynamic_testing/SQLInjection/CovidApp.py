...
class CovidApp():
    ...
    def validate_results_expect_error(self, response):
        ...
        print("status" + str(response['status']))
        assert response['status'] in (401, 403, 504, 500, 501, 400), "Bad HTTP status: " + str(response['status'])
    ...
    def public_unauthenticated_post_expect_error(self, endpoint, payload):
        conn = http.client.HTTPSConnection(self.pe_url)
        headers = {
            'content-type': "application/json",
            'cache-control': "no-cache"
    ...
    def user_consent_expect_error(self, code):
        payload = {}
        payload['accessCode'] = code
        payload['consent'] = True
        self.public_unauthenticated_post_expect_error("/consent", json.dumps(payload) )
    ...
    def upload_data_expect_error(self, code, json_data):
        payload = {}
        payload['accessCode'] = self.code
        payload['concernPoints'] = json_data
        result = self.public_unauthenticated_post_expect_error("/upload", json.dumps(payload))
        response = result['response'].read()
